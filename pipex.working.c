...
//     char *plus_equal_sign = find_plus_equal(arg);
//     char *key;
//     char *value;
...
static void setup_redirection(t_command *cmd, int *saved_stdout, t_shell *shell)
{
	if (cmd->output)
	{
...
static int execute_echo_builtin(t_command *cmd, t_shell *shell)
{
...
static int execute_pwd_builtin(t_command *cmd, t_shell *shell)
{
...
static int execute_cd_builtin(t_command *cmd, t_shell *shell)
{
...
static int execute_env_builtin(t_command *cmd, t_shell *shell)
{
...
static int execute_export_builtin(t_command *cmd, t_shell *shell)
{
...
static int execute_unset_builtin(t_command *cmd, t_shell *shell)
{
...
static int execute_exit_builtin(t_command *cmd, t_shell *shell)
{
...
static int execute_specific_builtin(t_command *cmd, t_shell *shell)
{
...
static int	count_env_vars(t_env_var *env_list)
{
	int	count;
...
static char	*create_env_var_string(const char *key, const char *value)
{
	char	*env_var;
	size_t	key_len;
	size_t	value_len;
	...
	env_var = malloc(key_len + value_len + 2); // +2 for '=' and '\0'
	if (!env_var)
		return (NULL);
...
static void	free_envp_on_error(char **envp, int i)
{
	while (i > 0)
		free(envp[--i]);
	free(envp);
...
// Function to prepare the environment for execve
// This function converts the linked list of environment variables to an array of strings
// char **prepare_envp(t_env_var *env_list) {
//     int env_count = 0;
//     t_env_var *current = env_list;
...
static char	*get_executable_path(char *cmd, t_env_var *env_list)
{
...
//     char **envp = prepare_envp(env_list);
//     execve(executable_path, cmd->argv, envp);
...
static void	handle_error(const char *msg, t_shell *shell)
{
	perror(msg);
	shell->exit_code = 1;
	exit(shell->exit_code);
...
static void	perform_redirection(int old_fd, int new_fd, const char *msg, t_shell *shell)
{
	if (old_fd != -1)
	{
...
static void	child_process(t_command *cmd, t_env_var *env_list, int input_fd, int output_fd, t_shell *shell)
{
...
static void	parent_process(pid_t pid, int input_fd, int output_fd, t_shell *shell)
{
	int	status;
...
//         int pipe_needed = create_pipe_if_needed(cmd, pipe_fd);
...
static void	perform_dup_and_close(int old_fd, int new_fd, const char *error_msg, t_shell *shell)
{
	if (old_fd != -1)
	{
		if (dup2(old_fd, new_fd) == -1)
...
// static void check_input_file(t_command *cmd, t_shell *shell)
// {
// 	int fd;
...
// static int create_pipe_if_needed(t_command *cmd, int pipe_fd[2])
// {
// 	if (cmd->next != NULL)
// 	{
// 		if (pipe(pipe_fd) == -1)
...
// static void setup_redirections(t_command *cmd, t_shell *shell, int input_fd, int pipe_fd[2])
// {
// 	if (input_fd != -1)
// 	{
// 		if (dup2(input_fd, STDIN_FILENO) == -1)
...
// static void handle_child_process(t_command *cmd, t_shell *shell, int input_fd, int pipe_fd[2])
// {
// 	setup_redirections(cmd, shell, input_fd, pipe_fd);
// 	if (handle_builtin(cmd, shell) == -1)
// 		execute_command(cmd, shell->env_list);
// 	exit(shell->exit_code);
// }

// static void close_unused_fds(int *input_fd, int pipe_fd[2])
// {
// 	if (*input_fd != -1)
// 		close(*input_fd);
// 	if (pipe_fd[1] != -1)
...
// 			handle_child_process(cmd, shell, input_fd, pipe_fd);
// 		else
// 			close_unused_fds(&input_fd, pipe_fd);
// 		cmd = cmd->next;
// 	}
// 	if (input_fd != -1)
// 		close(input_fd);
